...
class Drawing implements Mode{
    ...
    // Calculo el desplazamiento rápido en el eje Y.
    float offset_y = 0;
    float offset_z = 0;
    float offset_x = 0;
    ...
    void dibujar(ParticleSystem ps){
      ...
      ps.moveXYZ(offset_x,offset_y, offset_z);       // Es posible aplicar un offset.
    }
    ...
    void onkeyPressed(){
            ...
            switch (keyCode) {
              case UP:     {offset_y = - WORLD_SCALE * scale; break;}
              case DOWN:   {offset_y = WORLD_SCALE * scale; break;}
            }
    }
